...
export function PostActivity() {
  ...
  const [tags, setTags] = useState<string[]>(["ivvazh"]);
        ...
        <div className="grid grid-cols-1 items-start gap-12 lg:grid-cols-12">
          <section className="lg:col-span-7">
            ...
            <div className="space-y-6 rounded-xl border border-outline-variant bg-surface-container-lowest p-6 shadow-sm md:p-8">
              <div>
                <label htmlFor="title" className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">
                  Title
                </label>
                <input
                  id="title" type="text" maxLength={80} value={title}
                ...
                <div className="mt-1 flex justify-end">
                  <span className="font-label-mono text-[10px] text-outline">{title.length}/80</span>
                </div>
              </div>
              <div>
                <label className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">Category</label>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {categories.map((c) => {
                    const active = c.label === category.label;
                    return (
                      <button key={c.label} type="button" onClick={() => setCategory(c)}
                        className={"flex flex-col items-center gap-1 rounded-xl border p-4 transition-all focus:outline-none focus:ring-2 focus:ring-primary " +
                          (active ? "border-primary bg-primary-fixed" : "border-outline-variant hover:bg-surface-container-high")}
                      >
              ...
              <div>
                <label htmlFor="desc" className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">Description</label>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                  <label htmlFor="location" className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">Location</label>
                  <div className="relative">
                    ...
                    />
                    <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-primary">
                      <Icon name="my_location" size={20} />
                    </button>
                  </div>
                </div>
                <div>
                  <label htmlFor="datetime" className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">Date &amp; Time</label>
              ...
              <div className="grid grid-cols-1 items-end gap-6 md:grid-cols-2">
                <div>
                  <label htmlFor="slots" className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">Total Slots (2-20)</label>
                  <input id="slots" type="number" min={2} max={20} value={slots}
                ...
                <label className="flex h-[54px] items-center justify-between rounded-lg border border-outline-variant p-4">
                  ...
                  <span className="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox" className="peer sr-only" checked={anon} onChange={(e) => setAnon(e.target.checked)} />
              ...
              <div>
                <label className="mb-2 block font-label-mono uppercase tracking-wider text-on-surface-variant">Tags</label>
                <div className="mb-2 flex flex-wrap gap-1">
                  {tags.map((t) => (
                    <span key={t} className="inline-flex items-center gap-1 rounded-full bg-surface-container-high px-3 py-1 font-label-mono text-on-surface">
                      {t}
                      <button type="button" onClick={() => setTags(tags.filter((x) => x !== t))} className="hover:text-error">
              ...
              {isLoggedIn ? (
                ...
                >
                  <Icon name="add" /> Post Activity
                </button>
              ) : (
                <Link to="/login"
                  className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-4 font-headline text-on-primary shadow-lg transition-all hover:bg-primary-container active:scale-95"
                >
                  <Icon name="login" /> Sign in to Post Activity
                </Link>
              )}
            </div>
          </section>
          <aside className="lg:sticky lg:top-28 lg:col-span-5">
            <div className="mb-8">
              <h2 className="mb-2 font-headline text-headline-lg italic text-on-surface">Preview</h2>
            ...
            <article className="flex overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-md">
              ...
              <div className="flex-1 p-6">
                <div className="mb-4 flex items-start justify-between">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 overflow-hidden rounded-full border border-outline-variant bg-surface-container-high">
                      <img alt="" src={anon ? "https://api.dicebear.com/7.x/bottts-neutral/svg?seed=AnonHost" : "https://api.dicebear.com/7.x/avataaars/svg?seed=ivvazhUser"} />
                    </div>
                    <div>
                      <p className="font-headline text-sm italic leading-none">{anon ? "Anonymous Host" : "Your Name"}</p>
                ...
                <h3 className="mb-2 font-headline text-headline-md italic">{title || "The Activity Title"}</h3>
                <p className="mb-4 line-clamp-3 text-on-surface-variant">
                  {desc || "A brief, thoughtful description of your activity will appear here as you type..."}
                </p>
                <div className="mb-6 grid grid-cols-2 gap-2">
                  <div className="flex items-center gap-1 text-on-surface-variant">
                    <Icon name="location_on" size={18} /><span className="font-label-mono">{location || "Location"}</span>
                  </div>
                  <div className="flex items-center gap-1 text-on-surface-variant">
                    <Icon name="schedule" size={18} /><span className="font-label-mono">{previewTime}</span>
                  </div>
                </div>
                <div className="mb-6 flex flex-wrap gap-1">
                  {tags.map((t) => (
                    <span key={t} className="rounded-full bg-surface-container-high px-2 py-1 font-label-mono text-[10px] text-outline">{t}</span>
                  ))}
                </div>
                <div className="flex items-center justify-between border-t border-outline-variant pt-4">
                  <div className="flex gap-[2px]">
                    {Array.from({ length: slots }).map((_, i) => (
                      <span key={i} className={"h-2 w-2 " + (i === 0 ? "bg-secondary" : "border border-outline-variant")} />
                    ))}
                  </div>
            ...
            <div className="mt-6 flex items-start gap-3 rounded-xl bg-secondary-container p-6 text-on-secondary-container">
              ...
              <div>
                ...
                <p className="text-sm">Activities with a specific time and location get 3x more interest than vague ones.</p>
              </div>
            </div>
          </aside>
        </div>
