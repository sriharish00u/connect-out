...
import { requestOtp, verifyOtp, getProfile } from "@/lib/api/auth.functions";
...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in to IvVah to post or join small-group activities.",
      },
    ],
  }),
  component: Login,
...
function Login() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.isNewUser) {
        navigate({ to: "/onboarding", search: { phone } });
      } else {
        const profile = await getProfile({ data: { phone } });
        if (profile) {
          auth.login(profile);
          navigate({ to: "/" });
        } else {
      ...
      <main className="flex flex-grow items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          ...
          <p className="mt-6 text-center font-label-mono text-on-surface-variant">
            By continuing, you agree to IvVah&apos;s Guidelines &amp; Safety promise.
          </p>
